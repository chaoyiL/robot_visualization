...
def iter_items(obj):
    """Iterate over mapping-like objects that may lack .items()."""
...
class ReplayBuffer:
    ...
    def create_empty_zarr(cls, storage=None, root=None):
        if root is None:
            if storage is None:
                storage = zarr.MemoryStore() # Memory storage, use DirectoryStore for large data
    ...
    def get_episode_idxs(self):
        ...
        ends = np.asarray(self.episode_ends[:])
